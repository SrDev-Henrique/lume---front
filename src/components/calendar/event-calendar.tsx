...
export function EventCalendar({
  ...
  const handleEventSave = (event: CalendarEvent) => {
    if (event.id) {
      ...
      toast.custom((t) => (
        <Toast
          message={`Evento "${event.title}" adicionado`}
          onClick={() => toast.dismiss(t)}
        />
      ));
    }
  ...
  const handleEventDelete = (eventId: string) => {
    ...
    if (deletedEvent) {
      toast.custom((t) => (
        <Toast
          message={`Evento "${deletedEvent.title}" deletado`}
          onClick={() => toast.dismiss(t)}
        />
      ));
    }
  ...
  const handleEventUpdate = (updatedEvent: CalendarEvent) => {
    ...
    toast.custom((t) => (
      <Toast
        message={`Evento "${updatedEvent.title}" movido`}
        onClick={() => toast.dismiss(t)}
      />
    ));
  };
        ...
        >
          <div className="flex w-full items-center gap-1 sm:gap-[35%]">
            ...
            <div className="flex items-center sm:gap-3">
              ...
              <h2 className="font-medium text-sm sm:text-lg md:text-xl">
                {viewTitle}
              </h2>
            </div>
          </div>
          <div className="flex items-center gap-2">
            <DropdownMenu>
              ...
              <DropdownMenuContent
                align="end"
                className="min-w-32 bg-card backdrop-blur-md"
              >
